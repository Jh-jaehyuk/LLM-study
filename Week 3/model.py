...
class Transformer(nn.Module):
    ...
    def make_pad_mask(self, query, key):
        ...
        # ne는 not equal의 줄임말로 key 텐서에서 self.src_pad_idx와
        # 같은 값이 있는 부분은 False, 다른 값이 있는 부분은 True로 하여 리턴함
        key = key.ne(self.src_pad_idx).unsqueeze(1).unsqueeze(2)
        # (batch_size x 1 x 1 x src_token_len) 4

        # repeat는 각각의 차원을 따라 반복하여 텐서의 형태를 조절함
        # 아래와 같이 1, 1, len_query, 1 인 경우,
        # 0차원을 1번 반복, 1차원을 1번 반복, 2차원을 len_query번 반복, 3차원을 1번 반복하여
        # (batch_size, 1, 1, src_token_len)인 텐서를
        # (batch_size, 1, len_query, src_token_len) 형태로 만듦
